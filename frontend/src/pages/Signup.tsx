...
const Signup: React.FC = () => {
  const { signup, user, error: authError, setError } = useAuth();
  ...
  // Clear errors when mounting Signup component
  useEffect(() => {
    setError(null);
    if (user) {
      navigate('/');
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-600">
            Join CampusCart to start listing items and connecting with peers
          </p>
        </div>
